...
<body>
  ...
  <header>
    ...
    <div class="container-fluid" style="height: 450px; width: 400px; border: 1px goldenrod solid; margin-left: 35%;">
      <h3>C Language</h3><br><br>
      <div>
        <div>
          <p>Unit 1</p>
          <!-- <button onclick="viewNotes('C Language', 'Unit 1');">view</button><button>download</button> -->
          <button><a href="assest/notespdf/CL-U-1.pdf" style="text-decoration:none; color:black;">view</a></button><button onclick="downloadfile1()" id="downloadButton1">download</button>
        </div><br>
        <div>
          <p>Unit 2</p>
          <button><a href="assest/notespdf/CL-U-2.pdf" style="text-decoration:none; color:black;">view</a></button><button onclick="downloadfile2()" id="downloadButton2">download</button>
        </div><br>
        <div>
          <p>Unit 3</p>
    ...
    var pdfPath = 'assest/notespdf/CL-U-1.pdf';
    var link = document.createElement('a');

    link.href = pdfPath;
    link.download = 'downloaded_file_C_language.pdf';
    ...
    link.addEventListener('error', function () {
        console.error('Error occurred while attempting to download the file.');
    });
});
}
...
function submitForm() {
    ...
    // Use AJAX to fetch the PDF path
    $.ajax({
        type: "POST",
        url: "fetchdata.php",
        data: { subject: selectedSubject, unit: selectedUnit },
        success: function(pdfPath) {
            if (pdfPath && pdfPath !== "No data found") {
